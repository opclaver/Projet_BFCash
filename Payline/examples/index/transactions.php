...
<table style='border: 1px solid white;'>
	...
	while($line = fgets($handle)){
        ...
        $permalien = '/magic.do?reqCode=searchTransaction&mid='.MERCHANT_ID.'&ref='.$transactionInfo['MERCHANT_ID'].'&start='.date('Ymd:Hi',$transactionInfo['DATE']-3600).'&end='.date('Ymd:Hi',$transactionInfo['DATE']+3600);
        switch ($transactionInfo['ENVIRONMENT']) {
        	case paylineSDK::ENV_HOMO :
        		$permalien = paylineSDK::HOMO_CA.$permalien;
        		break;
        	case paylineSDK::ENV_PROD :
        		$permalien = paylineSDK::PROD_CA.$permalien;
        		break;
        }
        echo "<td style='padding:2px;'><a href='$permalien' target='CA'>".$transactionInfo['ORDER_REF'].'</a></td>'; // Order ref.
        $lienGetTD = "<a href='".KIT_ROOT.'examples/demos/extended.php?e=getTransactionDetails&transactionId='.$transactionInfo['TRANSACTION_ID']."'>".$transactionInfo['TRANSACTION_ID'].'</a>';
        echo "<td style='padding:2px;'>".$lienGetTD."</td>"; // Transaction ID
        echo "<td style='padding:2px;'>".$transactionType.'</td>'; // transaction type
        $imgResult =  KIT_ROOT.'examples/demos/img/cross.png';
        if(in_array($transactionInfo['RESULT'], array('00000','00100','02500','02501','03000','04003','34230','34330'))){
          $imgResult =  KIT_ROOT.'examples/demos/img/ok.png';
        }
        echo "<td style='padding:2px;'>".$transactionInfo['RESULT']."&nbsp<img height='15' src='$imgResult'></td>"; // result code
        $lienGetWPD = "<a href='".KIT_ROOT.'examples/demos/web.php?e=getWebPaymentDetails&token='.$transactionInfo['TOKEN']."'>".$transactionInfo['TOKEN'].'</a>';
        echo "<td style='padding:2px;'>".$lienGetWPD.'</td>'; // token
        $lienReset = "<a href='".KIT_ROOT.'examples/demos/direct.php?e=doReset&tid='.$transactionInfo['TRANSACTION_ID']."'><img alt='doReset' height='15' src='".KIT_ROOT."examples/demos/img/reset.png'></a>";
        $lienCapture = "<img alt='' height='20' src='".KIT_ROOT."examples/demos/img/filler.png'>";
        if(in_array($transactionInfo['ACTION'], array('100','110','120','RE'))){
          $lienCapture = "<a href='".KIT_ROOT.'examples/demos/direct.php?e=doCapture&tid='.$transactionInfo['TRANSACTION_ID'].'&contractNumber='.$transactionInfo['CONTRACT'].'&currency='.$transactionInfo['CURRENCY'].'&amount='.$transactionInfo['AMOUNT']."'><img alt='doCapture' height='20' src='".KIT_ROOT."examples/demos/img/capture.png'></a>";
        }
        $lienRefund = "<img alt='' height='20' src='".KIT_ROOT."examples/demos/img/filler.png'>";
        if(in_array($transactionInfo['ACTION'], array('100','110','120','101','111','121','204','RE','REV'))){
          $lienRefund = "<a href='".KIT_ROOT.'examples/demos/direct.php?e=doRefund&tid='.$transactionInfo['TRANSACTION_ID'].'&contractNumber='.$transactionInfo['CONTRACT'].'&currency='.$transactionInfo['CURRENCY'].'&amount='.$transactionInfo['AMOUNT']."'><img alt='doRefund' height='20' src='".KIT_ROOT."examples/demos/img/refund.png'></a>";
        }
