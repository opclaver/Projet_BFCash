<script type="text/javascript">
	//Requête AJAX pour sauvegarder l'adresse das le serveur commerçant et appeler Payline
	$(document).ready( function () { 
		$("#paymentForm").submit( function() {	// à la soumission du formulaire
			jQuery.support.cors = true; // requête ajax cross-domain
			var url = '<?php echo ENVIRONMENT == paylineSDK::ENV_HOMO ? paylineSDK::HOMO_GET_TOKEN_SERVLET : paylineSDK::PROD_GET_TOKEN_SERVLET ?> ';
			var data = "data="+$("#data").val() + "&accessKeyRef=<?php echo ACCESS_KEY_REF?>&cardNumber=" + $("#cardNumber").val() + "&cardExpirationDate=" + $("#cardExp").val() + ((document.getElementById("cardCvx").disabled == false) ? "&cardCvx=" + $("#cardCvx").val() : "");
